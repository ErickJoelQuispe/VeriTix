...
export function useTicketTypesRepository() {
  ...
  async function listByEvent(eventId: string): Promise<TicketType[]> {
    return apiRequest<TicketType[]>(`/admin/events/${eventId}/ticket-types`, { method: 'GET' })
  }

  async function create(eventId: string, payload: TicketTypePayload): Promise<TicketType> {
    return apiRequest<TicketType, TicketTypePayload>(`/admin/events/${eventId}/ticket-types`, {
      method: 'POST',
      body: payload,
    })
  }
  ...
  async function update(eventId: string, ticketTypeId: string, payload: Partial<TicketTypePayload>): Promise<TicketType> {
    return apiRequest<TicketType, Partial<TicketTypePayload>>(`/admin/events/${eventId}/ticket-types/${ticketTypeId}`, {
      method: 'PATCH',
      body: payload,
    })
  }
  ...
  async function remove(eventId: string, ticketTypeId: string): Promise<void> {
    await apiRequest(`/admin/events/${eventId}/ticket-types/${ticketTypeId}`, { method: 'DELETE' })
  }
