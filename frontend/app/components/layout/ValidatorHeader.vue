...
const validatorMenuItems = computed<AccountMenuItem[]>(() => {
  return [
    {
...
const navItems = [
  { label: 'Panel Validador', to: '/validator' },
  { label: 'Escanear QR', to: '/validator/scan' },
]
</script>
    ...
    <BaseContainer class="py-3.5 sm:py-4">
      <div class="grid gap-4 lg:grid-cols-[auto_minmax(0,1fr)_auto] lg:items-center lg:gap-8">
        ...
        >
          ...
          <span class="hidden text-sm font-medium text-muted sm:block">
            Panel Validador
          </span>
        </NuxtLink>

        <nav class="flex items-center justify-center" aria-label="Navegación validador">
